  ...
  User.findById(UserInfo._id).then((user) => {
    Course.findOne(CourseInfo)
      ...
      .catch(() => {
        return res
          .status(422)
          .send({ errors: { message: "Course not found" } });
      });
  });
};
...
const removelesson = (req, res, next) => {
  ...
  User.findById(UserInfo._id).then((user) => {
    Course.findById(CourseInfo._id)
      .then((course) => {
        if (!user) {
          return res
            .status(422)
        ...
        try {
          ...
          return res
            .status(422)
            .send({ errors: { message: "lesson couldn't be deleted" } });
        }
      })
      .catch(next);
  });
...
const lessonControler = { addlesson, removelesson };

module.exports = lessonControler;
